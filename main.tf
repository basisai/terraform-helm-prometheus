...
locals {
  ...
  server_values = {
    ...
    self_scrape_config = indent(6, yamlencode(local.self_scrape_config))
    scrape_configs     = indent(6, templatefile("${path.module}/templates/scrape_configs.yaml", local.scrape_config_values))
